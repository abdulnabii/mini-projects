...
export default function DiscoveryPage() {
  const [activeTab, setActiveTab] = useState<'feed' | 'inspect' | 'pipeline'>('feed');
  ...
  const [solverData, setSolverData] = useState<{
    ...
    language: string;
    isOpen: boolean;
  }>({ repoName: '', issueTitle: '', language: '', isOpen: false });
    ...
    <div className="space-y-10 font-mono w-full min-w-0">
      ...
      <div className="text-center space-y-3 max-w-3xl mx-auto">
        <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 text-xs">
          ...
          <span>AI OPEN SOURCE MATCHMAKER &amp; FIRST-PR ACCELERATOR</span>
        </div>
        ...
        <p className="text-slate-400 text-xs sm:text-sm font-mono max-w-2xl mx-auto leading-relaxed">
          Match your exact tech skills with healthy, responsive repositories. Audit any GitHub URL in real time, solve good first issues with Gemini 1.5, and track your PR milestones.
        </p>
      </div>
      ...
      <div className="flex items-center justify-center">
        <div className="p-1.5 rounded-2xl bg-[#0d1117] border border-slate-800 flex items-center gap-1.5 max-w-full overflow-x-auto shadow-xl">
          ...
          >
            ...
            <span>Smart Match Discovery Feed</span>
          </button>
          ...
          >
            ...
            <span>Audit Any GitHub Repo</span>
          </button>
          ...
          <button
            ...
            className={`px-5 py-2.5 rounded-xl text-xs font-bold transition-all flex items-center gap-2 whitespace-nowrap cursor-pointer ${
              activeTab === 'pipeline'
                ? 'bg-gradient-to-r from-emerald-400 to-teal-500 text-black font-black shadow-md'
                : 'text-slate-400 hover:text-white'
            }`}
          >
            <BookmarkCheck className="w-4 h-4" />
            <span>My PR Pipeline ({bookmarkedTargets.length})</span>
          </button>
        </div>
      </div>

      {/* TAB 1: SMART MATCH DISCOVERY FEED */}
      {activeTab === 'feed' && (
      ...
      {/* TAB 2: LIVE GITHUB REPO INSPECTOR */}
      {activeTab === 'inspect' && (
        <div className="animate-in fade-in duration-200">
          <RepoInspector
            onSelectProjectForGuide={(p) => setActiveGuideProject(p)}
            onSelectIssueForSolver={(repoName, issueTitle, language) =>
              setSolverData({ repoName, issueTitle, language, isOpen: true })
            }
          />
        </div>
      )}

      {/* TAB 3: MY PR CONTRIBUTION PIPELINE */}
      {activeTab === 'pipeline' && (
        <div className="space-y-6 animate-in fade-in duration-200">
          <div className="p-6 rounded-3xl bg-[#0d1117] border border-emerald-500/30 shadow-xl flex items-center justify-between">
            <div>
              <h3 className="text-lg font-bold text-white font-outfit">
                Live PR Kanban Pipeline
              </h3>
              <p className="text-xs text-slate-400">
                Keep track of your active forks, pull requests, and merged achievements
              </p>
            </div>
            <span className="px-3 py-1 rounded-full bg-emerald-500/10 text-emerald-300 text-xs font-bold border border-emerald-500/30">
              {bookmarkedTargets.filter((t) => t.status === 'merged').length} Merged PRs 🎉
            </span>
          </div>

          <ContributionPipeline
            targets={bookmarkedTargets}
            onUpdateStatus={(id, status) => {
              const updated = bookmarkedTargets.map((t) =>
            ...
            onRemoveTarget={(id) => {
              const updated = bookmarkedTargets.filter((t) => t.id !== id);
              setBookmarkedTargets(updated);
              localStorage.setItem(STORAGE_BOOKMARKS_KEY, JSON.stringify(updated));
            }}
          />
      ...
      {/* AI Issue Solver & PR Drafter Studio Modal */}
      <IssueSolverModal
        repoName={solverData.repoName}
        issueTitle={solverData.issueTitle}
        language={solverData.language}
        isOpen={solverData.isOpen}
        onClose={() => setSolverData((prev) => ({ ...prev, isOpen: false }))}
      />
    </div>
  );
}
