...
export default function SubjectOptimizer({ candidates, selectedIndex, onSelectSubject }: Props) {
  return (
    <div className="bg-[#131b2e] border border-indigo-500/20 rounded-3xl p-6 sm:p-8 space-y-5 font-mono">
      <div className="flex items-center justify-between border-b border-slate-800 pb-3">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">
            5 subject candidates ranked by predicted open rate algorithm. Click to select.
          </p>
        </div>
        <span className="text-xs text-emerald-400 font-bold">AI Analytics</span>
      </div>
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div className="space-y-0.5">
                  <span className="text-xs font-bold block">{item.subject}</span>
                  <div className="flex items-center gap-3 text-[10px] text-slate-400">
                    <span>Strategy: <strong className="text-indigo-400">{item.strategy}</strong></span>
                    <span>• {item.characterCount} chars</span>
                  </div>
                </div>
              </div>

              <div className={`px-3 py-1 rounded-xl border text-xs font-bold tabular-nums shrink-0 ${scoreColor}`}>
