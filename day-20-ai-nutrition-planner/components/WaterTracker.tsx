...
import { Droplet, Plus, Minus, RotateCcw, Sparkles, CheckCircle2 } from 'lucide-react';
...
export default function WaterTracker({ currentMl, targetMl, onAddWater, onResetWater }: Props) {
  ...
  return (
    <div className="p-6 rounded-3xl glass-card space-y-5 font-sans">
      <div className="flex items-center justify-between border-b border-white/[0.08] pb-3">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-400">Daily Target: {(targetMl / 1000).toFixed(1)}L</p>
          </div>
        </div>
      ...
      <div className="flex items-center gap-6">
        ...
        <div className="relative w-16 h-28 rounded-2xl bg-slate-950 border-2 border-cyan-500/40 overflow-hidden flex flex-col justify-end p-1 shadow-inner shrink-0">
          <div
            className="w-full rounded-xl bg-gradient-to-t from-cyan-600 via-cyan-400 to-sky-300 transition-all duration-700 ease-out shadow-lg shadow-cyan-500/30"
            style={{ height: `${percent}%` }}
          />
          <div className="absolute inset-0 flex items-center justify-center font-mono font-black text-xs text-white drop-shadow-md">
            {percent}%
          </div>
        </div>
        ...
        <div className="space-y-2 flex-1">
          ...
          <p className="text-xs text-slate-300 leading-tight">
            {percent >= 100
              ...
              : `${Math.max(0, targetMl - currentMl)}ml left to reach peak cellular energy.`}
          </p>
          ...
          <div className="flex items-center gap-1.5 pt-1 flex-wrap">
            {[
              ...
              <button
                ...
                className="px-2.5 py-1.5 rounded-xl bg-cyan-500/10 border border-cyan-500/30 hover:bg-cyan-500/20 text-cyan-300 font-bold text-xs transition-all cursor-pointer flex items-center gap-1 hover:scale-105"
              >
            ...
            <button
              ...
              className="p-1.5 rounded-xl bg-slate-950 border border-white/10 text-slate-400 hover:text-white disabled:opacity-30 disabled:cursor-not-allowed cursor-pointer"
              title="Undo 250ml"
            >
