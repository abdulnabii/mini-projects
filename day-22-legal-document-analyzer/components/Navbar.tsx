...
import { Scale, History, GitCompare, Sparkles, FileText, Plus } from 'lucide-react';

export default function Navbar() {
  const pathname = usePathname();

  return (
    <header className="sticky top-0 z-40 bg-[#060e14]/90 backdrop-blur-md border-b border-amber-500/20 font-mono text-xs">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
        {/* Brand */}
        <Link href="/" className="flex items-center gap-3 group">
          <div className="w-10 h-10 rounded-2xl bg-gradient-to-br from-amber-400 via-orange-500 to-rose-500 flex items-center justify-center text-black shadow-lg shadow-amber-500/20 group-hover:scale-105 transition-transform">
            <Scale className="w-5 h-5 fill-black font-black" />
          </div>
          <div>
            <div className="flex items-center gap-2">
              <span className="text-lg font-black tracking-tight text-white font-outfit">
                ClauseWise<span className="text-amber-400">.AI</span>
              </span>
            ...
            <p className="text-[10px] text-slate-400 font-mono hidden sm:block">
              AI Legal Document Analyzer &amp; Contract Risk Intelligence
            </p>
          </div>
        </Link>
        ...
        <div className="flex items-center gap-2.5 font-mono text-xs">
          ...
          >
            ...
            <span className="hidden sm:inline">Redline Diff</span>
          </Link>
